...
public class Balls
{
    ...
    public Balls(int amt, int armCount, List<Trail> trailList, Vector2 screen)
    {
        ...
        for (int i = 0; i < amt; i++)
        {
            ...
            orbits.Add(new PendjulumSim(armCount * amt, 20, screen, false, ballArms[i], trailList));
        }  
    }
    ...
    public void NewBall()
    {
        List<Vector2> alalathbar = new List<Vector2>();
            ...
            for (int j = 0; j < armCount; j++)
            {
                ...
                orbits[j].NewArm(true,balls[balls.Count-1].position);
            }
